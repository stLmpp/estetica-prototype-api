...
- **Never hand-edit `src/metadata.ts`.** It's auto-generated by the
  `@nestjs/swagger` CLI plugin (configured in `nest-cli.json`) during
  `nest build`/`nest start`, driven by whatever DTO/controller files exist
  on disk — never edit it directly, and don't rely on it being fully
  consistent while a build/watch process is mid-recompile.
