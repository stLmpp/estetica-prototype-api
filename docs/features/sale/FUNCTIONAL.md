...
- **Installments are only available on credit card, and are always
  generated by the API, never built by the caller.** Any transaction entry
  — one of sale creation's `transactions`, or the single transaction
  submitted to the add-transaction action on an existing sale — can carry
  an `installmentCount` instead of describing one transaction directly.
  When it does, that entry is a shorthand ("finance this amount over N
  installments") that expands into `installmentCount` separate `PAYMENT`
  transactions; the API rejects it unless the entry's payment method is
  `CREDIT_CARD`. The schedule is always monthly: installment *N* is due
  *N*-1 months after the entry's due date (same day each month). Splitting
  the total evenly rarely divides cleanly, so the last installment absorbs
  whatever's left over rather than distributing the remainder — the
  installments still always sum exactly to the entry's amount. The first
  installment can optionally be marked as already received (paid on the
  spot) as part of the same request; every other installment starts
  unconfirmed (`receivedAt` unset) and is confirmed later like any other
  transaction.
...
- **Installment payment at sale creation**
  - Given a sale of R$300 to be paid in 3x on a credit card, with a due
    date of the 15th
  - When the sale is created with a transaction entry of amount R$300,
    `installmentCount: 3`, and that due date
  - Then three `PAYMENT` transactions are created — R$100 due the 15th,
    ...
    are confirmed (`receivedAt` unset) unless the entry also marked the
    first as received

- **Installment plan set up after the sale already exists**
  - Given a `PENDING` sale with no transactions yet
  - When a transaction is added to it with `installmentCount: 3` (credit
    card)
  - Then the same expansion happens as at creation — three `PAYMENT`
    transactions are created against the existing sale, and its status is
    recomputed from all of them

- **Installment plan on an uneven split**
  - Given a transaction entry of R$100 to be paid in 3x on a credit card
  - When it's submitted (at creation or via add-transaction)
...
- **Attempt to finance a non-credit-card payment**
  - Given a transaction entry with `installmentCount` set whose payment
